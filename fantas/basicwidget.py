...
class MouseBase(Widget):
	...
	def __init__(self, ui, level=1):
		super().__init__(ui)
		self.level = level  # 表示抽象程度
		self.mousedown = None  # 标记按中的建
		self.events = [pygame.MOUSEBUTTONDOWN,
					   pygame.MOUSEBUTTONUP]
		if level > 1:
			self.events.append(pygame.MOUSEMOTION)
			self.mouseon = False
			if level == 3:
				self.events.append(pygame.MOUSEWHEEL)
	...
	def handle1(self, event):
		...
		if event.type == pygame.MOUSEBUTTONDOWN:
			...
			self.mousepress(event.pos)  # 按下
		elif event.type == pygame.MOUSEBUTTONUP:
			self.mouserelease(event.pos)  # 释放
